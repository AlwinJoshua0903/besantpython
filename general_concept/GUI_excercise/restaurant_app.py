...
lblinfo = Label(Tops, font=( 'aria' ,20, ),text=currenttime,fg="steel blue",anchor=W)
lblinfo.grid(row=1,column=0)
print(currenttime)
lblinfo = Label(bottoms, font=( 'aria' ,30, 'bold' ),text="contact us:1223445677",fg="steel blue",bd=10,anchor='e')
lblinfo.grid(row=0,column=0)
lblinfo = Label(bottoms, font=( 'aria' ,20, ),text='mail:abc@gmail.com',fg="steel blue",anchor=W)
lblinfo.grid(row=1,column=0)
text_Input=StringVar()
operator =""

txtdisplay = Entry(f2,font=('ariel' ,20,'bold'), textvariable=text_Input , bd=10,insertwidth=3 ,bg="white",justify='right')
...
def Ref():
    ...
    if fileexist:
        with open('hotel.csv','a',newline='') as file:
            ...
            writer.writerow([randomRef,cof,colfries,cob,cofi,cochee,
                            codr,costoffries,costoffilet,costofburger,
                            costoflargefries,costofcheeseburger,
                            costofdrinks,costofmeal,PayTax,Totalcost,
                            Ser_Charge,Service,OverAllCost,PaidTax,Service_Charge,
                             cost,Subtotal,Total])
    else:
        with open('hotel.csv','a',newline='') as file:
            writer=csv.writer(file)
